...
@simple_user_message_handler(
    settings_router, CustomCommandFilter("время бомбы ")
)
...
@simple_user_message_handler(
    settings_router, CustomCommandFilter("время удаления ")
)
    ...
    try:
        ...
        if time < 0:
            ...
            with open(config_path, "r") as f:
                content = loads(f.read())
            with open(config_path, "w") as f:
                content["delete_after"] = int(
                    event.object.object.text.split()[2]
                )
                f.write(dumps(content, indent=4))
            await edit_msg(
                api_session,
...
async def set_prefix(event: SimpleBotEvent):
    ...
    await edit_msg(
        ...
        event.peer_id,
        text=(
            f'Префикс изменится на "{content["prefix"]}" после перезагрузки'
            " бота "
        )
        + enabled,
    )
...
async def info_in_dm(event: SimpleBotEvent):
    ...
    if content["edit_or_delete"] == "edit":
        ...
        await edit_msg(
            ...
            event.object.object.message_id,
            event.peer_id,
            text="Теперь сообщения будут удаляться, а не редактироваться "
            + disabled,
        )
    else:
        ...
        await edit_msg(
            ...
            event.object.object.message_id,
            event.peer_id,
            text="Теперь сообщения будут редактироваться, а не удаляться "
            + enabled,
        )
